...
| # | Card | Band | Source or plan |
|---|---|---|---|
| 5.1 | Officer & director changes | **P** + **M** | 8-K **Item 5.02** gives event + date (**P**). **V4: the ROLE is available** — Form 3/4 `officerTitle` is parsed and stored (163,189 rows, 0 NULL), but needs structured columns because the composite string is ambiguous on 35% of values. The *action* verb stays **T**. |
...
| 5.4 | Insider transactions summary | **P** | `/insider-trades` — shipped, and now carries `is_derivative` and `rule_10b5_1`. |
| 5.5 | "N officers with 10b5-1 plans adopted in the trailing year" | **partial** | `rule_10b5_1` gives *how many trades were made under a plan*, not the adoption date (D-10b5-1). **V3 adds `ecd:Rule10b51ArrAdoptedFlag`/`…TrmntdFlag`** — whether *any* officer adopted or terminated an arrangement in the period. **Still a period boolean, not a date: D-10b5-1 stands.** The copy must not claim "adopted in the trailing year". |
