...
| # | Card | Band | Source or plan |
|---|---|---|---|
| 5.1 | Officer & director changes | ✅ **DONE 2026-08-04** | `GET /officer-changes` — Form 3 arrivals (person + role) interleaved with 8-K Item 5.02 events (date), **never joined**. Structured role columns added (`officer_title`, `is_director`, `is_officer`, `is_ten_percent_owner`); legacy rows fall back to the display string's left prefix, which is **exact** (12,630 distinct values, 0 violations). **The action verb is confirmed unavailable** — EDGAR's item code carries no sub-item letter, so the card has no action column. Officers + directors only; 10% owners and `other` excluded with the count reported. **Expanded 2026-08-04:** adds the current officer/director **roster** (who they are, per the role each last reported) and **role-box transitions** (665 pure additions; removals skipped because a box disappearing is indistinguishable from a filer omitting it). |
| 5.2 | Board composition — size / independent / tenure / CEO tenure | **X** | **V2-verified absent** from the tagged DEF 14A. |
