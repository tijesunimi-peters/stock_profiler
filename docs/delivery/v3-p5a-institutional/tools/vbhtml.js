...
const {URL:U,SEL,NAV,OPEN,LABEL,NTH="0",N="0",PRE}=process.env;
...
if(PRE)await p.evaluate((sel,label)=>{const r=document.querySelector(sel);const e=[...r.querySelectorAll("button")].find(e=>(e.innerText||"").replace(/\s+/g," ").trim()===label);if(e)e.click();},SEL,PRE);
if(PRE)await wait(1500);
...
console.log(await p.evaluate((n)=>{
  ...
  const root=ov||document;
  const s=root.querySelectorAll("svg")[n];
