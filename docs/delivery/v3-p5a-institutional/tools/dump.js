// click a control, then dump the outerHTML of a named container
const puppeteer=require("puppeteer");
const {URL:U,SEL,NAV,OPEN,LABEL,NTH="0",PICK}=process.env;
...
if(LABEL){await p.evaluate((sel,label,nth)=>{const r=document.querySelector(sel);const all=[...r.querySelectorAll("button,a,[role=button]")].filter(e=>(e.innerText||"").replace(/\s+/g," ").trim()===label);if(all[nth])all[nth].click();},SEL,LABEL,+NTH);await wait(1500);}
console.log(await p.evaluate((sel,pick)=>{
  const root=document.querySelector(sel);
  const el=[...root.querySelectorAll("*")].find(e=>(e.textContent||"").replace(/\s+/g," ").trim().startsWith(pick));
  return el?el.outerHTML.slice(0,60000):"NOT FOUND";
},SEL,PICK));
