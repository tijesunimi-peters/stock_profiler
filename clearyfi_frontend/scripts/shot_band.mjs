/* Screenshot the "What changed this filing" band for eyeballing. */
...
const PORT=5198, API="http://127.0.0.1:8000";
...
for (const w of [1440, 420]) {
  ...
  for (const TK of (process.env.TICKERS||"AAPL").split(",")) {
    ...
    if (band) await band.screenshot({path: join(OUT, `band-${TK}-${w}.png`)});
  }
  await p.close();
}
