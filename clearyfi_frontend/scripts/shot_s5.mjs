/* Screenshot §05 (governance & people) for eyeballing — the insider card in particular. */
...
const PORT=5192, API="http://127.0.0.1:8000";
...
for (const w of [1440, 420]) {
  ...
  for (const TK of (process.env.TICKERS||"AAPL").split(",")) {
    ...
    await p.waitForFunction(()=>[...document.querySelectorAll(".p-card")].some(e=>/Insider transactions/i.test(e.textContent||"")),{timeout:40000});
    await new Promise(r=>setTimeout(r,800));
    const sec = await p.evaluateHandle(()=>document.querySelector("#s5")?.closest(".hub-sec"));
    await sec.asElement().screenshot({path: join(OUT, `s5-${TK}-${w}.png`)});
  }
  await p.close();
}
