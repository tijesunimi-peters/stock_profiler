/* Screenshot §03 (segments & geography) for eyeballing. */
...
const PORT=5200, API="http://127.0.0.1:8000";
...
for (const w of [1440, 420]) {
  ...
  for (const TK of (process.env.TICKERS||"AAPL").split(",")) {
    ...
    const band = await p.evaluateHandle(()=>document.querySelector("#s3")?.closest(".hub-sec")).then(h=>h.asElement());
    if (band) await band.screenshot({path: join(OUT, `s3-${TK}-${w}.png`)});
  }
  await p.close();
}
