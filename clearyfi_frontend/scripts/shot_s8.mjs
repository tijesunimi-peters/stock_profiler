/* Screenshot §08 (filing activity & disclosure events) for eyeballing. */
...
const PORT=5197, API="http://127.0.0.1:8000";
...
for (const w of [1440, 420]) {
  ...
  for (const TK of (process.env.TICKERS||"AAPL").split(",")) {
    ...
    await p.waitForFunction(()=>[...document.querySelectorAll(".p-card")].some(e=>/8-K disclosure profile/i.test(e.textContent||"")),{timeout:40000});
    await new Promise(r=>setTimeout(r,800));
    const sec = await p.evaluateHandle(()=>document.querySelector("#s8")?.closest(".hub-sec"));
    await sec.asElement().screenshot({path: join(OUT, `s8-${TK}-${w}.png`)});
  }
  await p.close();
}
