      ...
      <div className="stmt-wrap">
        <table className="stmt-table">
          <thead>
            <tr>
              <th>{labelHeader}</th>
              <th className="amt">{amountHeader}</th>
              <th>Source</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row, i) => (
              <tr key={i}>
                <td className="stmt-label">{row.label}</td>
                <td className="amt">
                  <span
                    className={["stmt-amt", row.drained ? "drained" : null]
                      .filter(Boolean)
                      .join(" ")}
                  >
                    {row.amount}
                ...
                <td>
                  {row.sourceTag ? (
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 7 }}>
                      <span className="stmt-tag">{row.sourceTag}</span>
                      <SourceBadge kind={row.isExtension ? "ext" : "gaap"} />
                    </span>
                  ) : null}
                </td>
              </tr>
